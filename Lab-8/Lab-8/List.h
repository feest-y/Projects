...
List* Create(short n = 1) //функция создания списка (возвращает адрес его головы)
{
	if (n == 0)
		return head;
	...
	List* p, * pred; // pred – указатель на предыдущую структуру
	head = p = pred = (List*)malloc(sizeof(List)); //выделяем память для первой записи
